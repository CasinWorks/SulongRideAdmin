...
extension.build_configurations.each do |config|
  ...
  config.build_settings['INFOPLIST_KEY_CFBundleDisplayName'] = 'Sulong Driver'
  ...
  config.build_settings['PRODUCT_BUNDLE_IDENTIFIER'] = 'com.etrikeph.etrikePhDriver.TripLiveActivityExtension'
...
swift_path = 'TripLiveActivity/TripLiveActivity.swift'
swift_file = project.files.find { |f| f.path == swift_path } || project.new_file(swift_path)
...
assets_path = 'TripLiveActivity/Assets.xcassets'
assets = project.files.find { |f| f.path == assets_path } || project.new_file(assets_path)
