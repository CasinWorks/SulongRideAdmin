...
final messageNotificationsProvider = Provider<void>((ref) {
  final coordinator = _MessageNotificationCoordinator(
    ref,
    activeTripProvider: riderActiveTripProvider,
    pushEnabled: EcoLocalStore.pushNotifications,
    peerLabel: 'Driver',
  );
...
class _MessageNotificationCoordinator {
  ...
  void _bindTrip(TripModel? trip) {
    ...
    if (tripId == null) {
      unawaited(TripLiveActivityService.end());
      return;
    }
  ...
  Future<void> _onMessages(List<MessageModel> messages, String tripId) async {
    ...
    for (final message in messages) {
      ...
      final trip = _trip;
      if (trip != null && tripChatIsOpen(trip.status)) {
        await TripLiveActivityService.showChatPreview(
          trip: trip,
          preview: preview,
        );
      }
