...
private let appGroupId = "group.com.etrikeph.etrikePhUser"
...
/// Lock Screen (all iPhones 16.1+) and Apple Watch Smart Stack (watchOS 11+, iOS 18+).
@available(iOSApplicationExtension 16.1, *)
...
/// Compact layout for Apple Watch Smart Stack — mirrors booking progress from iPhone.
@available(iOSApplicationExtension 18.0, *)
...
private func tripTitle(_ context: ActivityViewContext<LiveActivitiesAppAttributes>) -> String {
  sharedDefault?.string(forKey: context.attributes.prefixedKey("title")) ?? "Sulong Ride"
}
